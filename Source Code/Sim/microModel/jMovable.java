...
public abstract class jMovable {
    ...
    public void updateNeighbours() {
        ...
        // left neighbours
        if (lane.left!=null) {
            xAdj = getAdjacentX(jModel.latDirection.LEFT);
             ...
             * checks. Instead, refind the vehicle if appropiate.
             */ 
            ...
            if (leftUp==null) {
                ...
                leftUp = lane.left.findVehicle(xAdj, jModel.longDirection.UP);
            } else {
                // if there is a leftUp, check overtake
                jLane adjlane = lane.left;
            ...
            if (leftUp!=null && leftUp.lane.downSplit==lane.left.downSplit) {
                ...
                leftDown = lane.left.findVehicle(xAdj, jModel.longDirection.DOWN);
        ...
        // right neighbours
        if (lane.right!=null) {
            xAdj = getAdjacentX(jModel.latDirection.RIGHT);
            /* Remove adjacent neighbour in case of split, as this vehicle may
             * split the other way and become untreatable for regular overtake 
             * checks. Instead, refind the vehicle if appropiate.
             */
            ...
            if (rightUp==null) {
                ...
                rightUp = lane.right.findVehicle(xAdj, jModel.longDirection.UP);
            } else {
                // if there is a rightUp, check overtake
                jLane adjlane = lane.right;
                // search upstream while neighbour is downstream
            ...
            if (rightUp!=null && rightUp.lane.downSplit==lane.right.downSplit) {
                ...
                rightDown = lane.right.findVehicle(xAdj, jModel.longDirection.DOWN);
                // one vehicle, at a perfectly equal coordinate, may be in between
    ...
    public void updateNeighboursInv() {
         ...
         */
        pasteAsAdjacentLeader(lane, jModel.longDirection.DOWN);
        pasteAsAdjacentLeader(lane, jModel.longDirection.UP);
    }
     ...
     */
    public double getAdjacentX(jModel.latDirection dir) {
     ...
     */
    public double getHeadway(jMovable leader) {
        ...
        if (lane==leader.lane) {
            ...
            // leader is right
            s = leader.getAdjacentX(jModel.latDirection.LEFT) - x;
        } else if (lane==leader.lane.right) {
            // leader is left
            s = leader.getAdjacentX(jModel.latDirection.RIGHT) - x;
    ...
    public void delete() {
        ...
        } else if (this instanceof jLcVehicle) {
            jLcVehicle veh = (jLcVehicle) this;
            // model
            model.removeVehicle(veh);
            // vehicle
    ...
    public void cut() {
        ...
        if (down==null && lane.downSplit!=null) {
            java.util.ArrayList<jMovable> downs = findVehiclesDownstreamOfSplit(lane);
            for (jMovable d : downs) {
                if (d.up==this) {
                    d.up = up;
                }
            }
        ...
        if (up==null && lane.upMerge!=null) {
            java.util.ArrayList<jMovable> ups = findVehiclesUpstreamOfMerge(lane);
            for (jMovable d : ups) {
                if (d.down==this) {
                    d.down = down;
                }
         ...
         * lanes.
         */
        cutAsAdjacentLeader(lane, jModel.longDirection.DOWN);
        cutAsAdjacentLeader(lane, jModel.longDirection.UP);
        ...
        if (x>lane.l) {
            if (lane.right!=null) {
                jMovable veh = lane.right.findVehicle(lane.right.l, jModel.longDirection.UP);
            ...
            if (lane.left!=null) {
                jMovable veh = lane.left.findVehicle(lane.left.l, jModel.longDirection.UP);
     ...
     */
    protected java.util.ArrayList<jMovable> findVehiclesUpstreamOfMerge(jLane merge) {
        java.util.ArrayList<jMovable> out = new java.util.ArrayList<jMovable>();
        for (jLane j : merge.upMerge.ups) {
            jMovable d = j.findVehicle(j.l, jModel.longDirection.UP);
            if (d!=null) {
                out.add(d);
     ...
     */
    protected java.util.ArrayList<jMovable> findVehiclesDownstreamOfSplit(jLane split) {
        java.util.ArrayList<jMovable> out = new java.util.ArrayList<jMovable>();
        for (jLane j : split.downSplit.downs) {
            jMovable d = j.findVehicle(0, jModel.longDirection.DOWN);
            if (d!=null) {
                out.add(d);
     ...
     */
    protected void cutAsAdjacentLeader(jLane k, jModel.longDirection lon) {
        if (jModel.longDirection.DOWN==lon) {
            for (jLane n : k.downs) {
     ...
     * @param lon Longitudinal direction.
     */
    protected void cutAsAdjacentLeader0(jLane k, jModel.longDirection lon) {
        jMovable m = null;
        if (jModel.longDirection.DOWN==lon) {
            // look downstream
            if (k.right!=null) { // look right
                m = k.right.findVehicle(0, jModel.longDirection.DOWN);
            ...
            if (k.left!=null) { // look left
                m = k.left.findVehicle(0, jModel.longDirection.DOWN);
            ...
            // look upstream
            if (k.right!=null) { // look right
                m = k.right.findVehicle(k.right.l, jModel.longDirection.UP);
            ...
            if (k.left!=null) { // look left
                m = k.left.findVehicle(k.left.l, jModel.longDirection.UP);
     ...
     * @param lon Longitudinal direction.
     */
    protected void pasteAsAdjacentLeader(jLane k, jModel.longDirection lon) {
        if (jModel.longDirection.DOWN==lon) {
            for (jLane n : k.downs) {
     ...
     * @param lon Longitudinal direction.
     */
    protected void pasteAsAdjacentLeader0(jLane k, jModel.longDirection lon) {
        jMovable m = null;
        if (jModel.longDirection.DOWN==lon) {
            // look downstream
            if (k.right!=null) { // look right
    ...
    public void paste(jLane atLane, double atX) {
        ...
        up = atLane.findVehicle(atX, jModel.longDirection.UP);
        if (up!=null) {
            ...
            if (up.down==null && up.lane.downSplit!=atLane.downSplit) {
                ...
                down = atLane.findVehicle(atX, jModel.longDirection.DOWN);
            } else {
