...
public class jModel {
     ...
     * @param vehicle Vehicle to check existance of.
     * @return Whether vehicle exists in simulation.
     */
    public boolean exists(jMovable vehicle) {
        if (vehicle instanceof jVehicle) {
            return vehicles.contains((jVehicle) vehicle);
        } else {
            return lcVehicles.contains((jLcVehicle) vehicle);
        }
    }
    ...
    public java.util.ArrayList<jVehicle> getVehicles() {
        return (java.util.ArrayList<jVehicle>) vehicles.clone();
    }
    ...
    public void storeData() {
        ...
        // Run on-baord units, road-side units and controllers
        runUnits();
     ...
     */
    public static jTrajectoryData loadTrajectoryData(String file) {
        jTrajectoryData out = null;
        try {
            out = (jTrajectoryData) loadData(file);
    ...
    public void saveDetectorData(jDetector detector) {
        saveData(new jDetectorData(detector), "detectors", "detector"+detector.id()+".dat");
    }
     ...
     */
    public static jDetectorData loadDetectorData(String file) {
        jDetectorData out = null;
        try {
            out = (jDetectorData) loadData(file);
        } catch (java.lang.ClassCastException cce) {
    ...
    public void saveData(Object obj, String subPath, String fileName) {
        try {
            ...
            if (subPath==null || subPath.isEmpty()) {
                tmpFile = new java.io.File(settings.getString("outputDir"), fileName);
            } else {
                tmpFile = new java.io.File(settings.getString("outputDir"), subPath);
                tmpFile = new java.io.File(tmpFile, fileName);
            }
            boolean dirsCreated = tmpFile.getParentFile().mkdirs();
            boolean newFileCreated = tmpFile.createNewFile();
