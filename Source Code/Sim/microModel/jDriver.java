...
public class jDriver {
    ...
    protected java.util.HashMap<jConflict.conflictRSU, jMovable> conflictYieldPlans = 
            new java.util.HashMap<jConflict.conflictRSU, jMovable>();
     ...
     */
    protected java.util.ArrayList<jConflict.conflictRSU> keepClearConflicts = 
            new java.util.ArrayList<jConflict.conflictRSU>();
    ...
    /** Allowed deceleration for dead-end. */
    public double bDeadend = 5;
    ...
    public void drive() {
        ...
        if (vehicle.lcProgress == 0) {
            ...
            if (vehicle.x>100 || vehicle.lane.generator==null) {
                ...
                } else if (vehicle.lane.left!=null && vehicle.lane.left.upMerge!=null &&
                        vehicle.lane.left.upMerge.xAdj(vehicle.lane.left)+
                        vehicle.getAdjacentX(jModel.latDirection.LEFT) < vehicle.lane.left.getVLim()*Tmax) {
                    // no left upstream vehicle, but there is a nearby merge, maybe not ok
                    aFollow = Double.NEGATIVE_INFINITY;
                }
                ...
                } else if (vehicle.lane.right!=null && vehicle.lane.right.upMerge!=null &&
                        vehicle.lane.right.upMerge.xAdj(vehicle.lane.right)+
                        vehicle.getAdjacentX(jModel.latDirection.RIGHT) < vehicle.lane.right.getVLim()*Tmax) {
                    // no right upstream vehicle, but there is a nearby merge, maybe not ok
                    aFollow = Double.NEGATIVE_INFINITY;
                }
     ...
     * @return Desired velocity [m/s]
     */
    public double desiredVelocity(jLane lane) {
    ...
    public void noticeRSUs() {
        ...
        for (jRSU rsu : vehicle.RSUsInRange) {
            if (rsu.noticeable) {
                notice(rsu);
            }
        }
     ...
     * general notice which will invoke the appropiate notice method.
     * @param rsu Noticed RSU.
     */
    public final void notice(jRSU rsu) {
        try {
            java.lang.reflect.Method method = getClass().getMethod("notice", rsu.getClass());
    ...
    /**
     * Reduce speed such that the speed is equal to the appropiate speed at the 
     * location of the speed reduction once that location is reached.
     * @param sr Speed reduction.
     */
    public void notice(jSpeedReduction sr) {
     ...
     */
    public void notice(jLane.splitRSU split) {
        if (vehicle.down==null) {
            // get appropiate lane
            jLane lane = split.getLaneForRoute(vehicle.route);
            // lane may be null if not appropiate for the route (will change lane before)
            if (lane!=null) {
                // follow downstream vehicle
                jMovable down = lane.findVehicle(0, jModel.longDirection.DOWN);
                if (down!=null) {
     ...
     */
    public void notice(jConflict.conflictRSU conflict) {

        // Notice as speed reduction
        notice((jSpeedReduction) conflict);
            ...
            while (up!=null && loop) {
                ...
                if (s>-up.l && sOther<conflict.length()) {
                    ...
                    if (sOther<0 && up.up==null && conflict.isMerge()) {
                        ...
                        jMovable up2 = conflict.otherRSU().lane.findVehicle(
                                conflict.otherRSU().lane.l, jModel.longDirection.UP);
     ...
     */
    protected double anticipateConflictMovement(double s, double v, double a) {
        if (s<=0) {
     ...
     */
    protected void stopForConflict(jConflict.conflictRSU conflict, boolean safe, boolean stopUpstream) {
        
        // If requested, stop for upstream conflicts that must be kept clear.
        if (stopUpstream) {
            for (jConflict.conflictRSU rsu : keepClearConflicts) {
     ...
     */
    public void noticeIntersection(jRSU rsu) {
        ...
        if (vehicle.down!=null) {
            ...
            if (vehicle.lane.goLeft && vehicle.route.canBeFollowedFrom(vehicle.lane.left) && 
                    ...
                    vehicle.lane.getAdjacentX(vehicle.x, jModel.latDirection.LEFT)) > x) ) {
            ...
            if (vehicle.lane.goRight && vehicle.route.canBeFollowedFrom(vehicle.lane.right) && 
                    ...
                    vehicle.lane.getAdjacentX(vehicle.x, jModel.latDirection.RIGHT)) > x) ) {
